...
const groupSubItems = [
  { label: "Guruhlar", path: "/dashboard/groups" },
  { label: "Yig'ilayotgan guruhlar", path: "/dashboard/planned-groups" },
];
...
function Sidebar({ collapsed }) {
  ...
  const groupActive = groupSubItems.some(
  ...
  useEffect(() => {
    if (groupActive) setGroupsOpen(true);
  }, [groupActive]);

  const handleGroupHeader = () => {
    if (collapsed) {
      navigate("/dashboard/groups");
      return;
    }
    setGroupsOpen((value) => !value);
  };

  const solidActive = {
    bgcolor: purple,
    color: "#fff",
    "&:hover": { bgcolor: "#684bcf", color: "#fff" },
  };
  const inactiveItem = {
    color: "text.secondary",
    "&:hover": { bgcolor: "action.hover", color: purple },
  };
      ...
      <Box sx={{ px: 1.25, display: "flex", flexDirection: "column", gap: 0.8 }}>
        <Tooltip title={collapsed ? "Guruhlar" : ""} placement="right" arrow>
          <Box
            role="button"
            tabIndex={0}
            onClick={handleGroupHeader}
            onKeyDown={(event) => {
              if (event.key === "Enter" || event.key === " ") handleGroupHeader();
            }}
            sx={{
              height: 54,
              px: collapsed ? 0 : 2,
              ...
              cursor: "pointer",
              outline: "none",
              color: groupActive ? purple : "text.primary",
              "&:hover": { bgcolor: "action.hover", color: purple },
            }}
          >
            ...
            {!collapsed && (
              <>
                <Typography sx={{ flex: 1, fontSize: 15, fontWeight: 700 }}>Guruhlar</Typography>
                {groupsOpen ? <FiChevronUp size={20} /> : <FiChevronDown size={20} />}
              </>
            )}
          </Box>
        </Tooltip>
            ...
            return (
              <Box
                ...
                sx={{
                  ...
                  ...(active ? solidActive : inactiveItem),
                }}
              >
                {item.label}
              </Box>
        ...
        <Tooltip title={collapsed ? "Profil" : ""} placement="right" arrow>
          <Box
            ...
            onKeyDown={(event) => {
              if (event.key === "Enter" || event.key === " ") navigate("/dashboard/profile");
            }}
            sx={{
              height: 54,
              px: collapsed ? 0 : 2,
              ...
              fontWeight: 600,
              transition: "background-color 160ms ease, color 160ms ease",
              ...(profileActive ? solidActive : inactiveItem),
            }}
          >
            <FiUser size={20} />
            {!collapsed && <Typography sx={{ fontSize: 15, fontWeight: 600 }}>Profil</Typography>}
          </Box>
        </Tooltip>
      </Box>

      <Box sx={{ flex: 1 }} />
      {isDark && <Box sx={{ height: 1 }} />}
    </Paper>
  );
}

function Topbar({ profile, collapsed, onToggle }) {
  const theme = useTheme();
      ...
      <Box sx={{ display: "flex", alignItems: "center", gap: 1.4, minWidth: 0 }}>
        <IconButton
          ...
          sx={{
            ...
            bgcolor: purple,
            color: "#fff",
            boxShadow: "0 2px 8px rgba(116, 86, 216, 0.4)",
        ...
        >
          ...
          >
            <FaLayerGroup size={18} color={purple} />
            Guruhlarim
          </MenuItem>
        </Menu>
